<template>
  <el-container>
    ...
    <el-input v-model="form.name" ></el-input>
        ...
        <!-- <el-tab-pane label="服务器对接" name="second">
          <el-form ref="form" :model="form" label-width="350px">
            <el-form-item label="" style="width:40%" />
            <el-form-item label="公众号设置 (微信后台 > 公众号设置 > 功能设置)" style="width:40%" />
            <el-form-item label="URL(服务器地址)" style="width:700px">
              <el-input v-model="jiexiurlzifucuan.hostname" disabled />
            </el-form-item>
            <el-form-item label="业务域名"  style="width:700px">
              <el-input v-model="jiexiurlzifucuan.hostname" disabled />
            </el-form-item>
            <el-form-item label="JS接口安全域名" style="width:700px">
              <el-input v-model="jiexiurlzifucuan.hostname" disabled />
            </el-form-item>
            <el-form-item label="网页授权域名"  style="width:700px">
              <el-input v-model="jiexiurlzifucuan.hostname" disabled />
            </el-form-item>
            <el-form-item>
            </el-form-item>
...
<script>
import { wechatUpdate } from '@/api/wechat.js'
...
export default {
  ...
  created() {
    ...
    var search = url.slice(14)
    console.log(search)
    const shuju = querystring.parse(search, '&', '=')
    this.form = shuju
    this.kefuerweima = 'http://' + this.form.qrcode_thumb_url.slice(0)
    this.guanzhuerweima = 'http://' + this.form.sub_thumb_id.slice(0)
    console.log(this.kefuerweima)
    console.log(this.guanzhuerweima)
    const jiexiurl = require('url')
    this.jiexiurlzifucuan = jiexiurl.parse(this.form.url)
    console.log(this.jiexiurlzifucuan.hostname)
    // 以上解析url获取从上一个页面传来的参数，以下获取子账号列表
    zizhanghaohuoqv(this.page).then(response => {
      console.log(response)
      this.zizhanghaoliebiao = response.data.data
    })
    this.value = this.form.manager_id
  },
  methods: {
    baocun() {
      this.form.manager_id = this.value
      wechatUpdate(this.form).then(response => {
        console.log(this.form)
        console.log(response)
        if (response.message === 'sucessfull!') { alert('修改成功'); this.fanhui() }
      })
    },
    ...
    handleAvatarSuccess(res, file) {
      ...
      this.kefuerweima = 'http://' + res.data.url.slice(1)
      this.$message('上传成功')
    },
